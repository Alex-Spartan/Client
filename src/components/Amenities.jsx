...
const Amenities = ({ data, setData }) => {
  ...
  const addCheckbox = (e) => {
    ...
    if (checked) {
      ...
      setData({
        ...data,
        amenities: data.amenities.filter((item) => item !== name),
      });
    }
  };
  ...
  return (
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-4">
      <label className="flex justify-start items-center border border-gray-600 p-3 gap-2 rounded-xl md:p-6 md:gap-3">
        <input
          type="checkbox"
          checked={data.amenities.includes("WiFi")}
          name="WiFi"
      ...
      <label className="flex justify-start items-center border border-gray-600 p-3 gap-2 rounded-xl md:p-6 md:gap-3">
        <input
          type="checkbox"
          checked={data.amenities.includes("Housekeeping")}
          name="Housekeeping"
          onChange={addCheckbox}
        />
        <IoTrashBinOutline />
      ...
      <label className="flex justify-start items-center border border-gray-600 p-3 gap-2 rounded-xl md:p-6 md:gap-3">
        <input
          type="checkbox"
          checked={data.amenities.includes("24-hour security")}
      ...
      <label className="flex justify-start items-center border border-gray-600 p-3 gap-2 rounded-xl md:p-6 md:gap-3">
        <input
          type="checkbox"
          checked={data.amenities.includes("Laundry")}
          name="Laundry"
          onChange={addCheckbox}
        />
        <IoShieldHalfOutline />
      ...
      <label className="flex justify-start items-center border border-gray-600 p-3 gap-2 rounded-xl md:p-6 md:gap-3">
        <input
          type="checkbox"
          checked={data.amenities.includes("Parking")}
          name="Parking"
          onChange={addCheckbox}
        />
        <IoCar />
